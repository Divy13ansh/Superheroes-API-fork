...
class SuperheroViewSet(ModelViewSet):
    ...
    def top_superheroes(self, request):
        ...
        superheroes = self.queryset.filter(is_villain=False).order_by('-power_level', 'name')[:limit]
        serializer = SuperheroListSerializer(superheroes, many=True)
        return Response(serializer.data)
...
class SuperheroStatsView(APIView):
    ...
    def get(self, request):
        ...
        # Average power level
        avg_power = Superhero.objects.aggregate(avg_power=Avg('power_level'))['avg_power'] or 0
        
        # Universe distribution
        universe_stats = Superhero.objects.values('universe').annotate(
            count=Count('id')
        ).order_by('-count')
        universe_distribution = {stat['universe']: stat['count'] for stat in universe_stats}
        
        # Power level distribution
        power_stats = Superhero.objects.values('power_level').annotate(
            count=Count('id')
        ).order_by('power_level')
        power_level_distribution = {str(stat['power_level']): stat['count'] for stat in power_stats}
